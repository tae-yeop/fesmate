...
#### 5.5.1 나만의 타임테이블 (My Timetable)

> 타임테이블 탭 상단에 **[📋 내 타임테이블]** 버튼으로 진입

**플로우**
```mermaid
graph TD
  A[타임테이블 탭] --> B{⭐체크한 슬롯 있음?}
  B -->|Yes| C[내 타임테이블 뷰]
  B -->|No| D["빈 상태: '보고싶은 공연을 체크해보세요'"]
  C --> E[커스텀 이벤트 추가]
  C --> F[공유 링크 생성]
  F --> G[친구에게 링크 전달]
  G --> H[친구 타임테이블 추가]
...
│         └ 내가 추가한 일정           │
│                                     │
│  13:00  ⭐ 아티스트A - Main Stage    │
│         └ 슬롯 체크                  │
│                                     │
│  14:00  ⭐ 아티스트B - Main Stage    │ ← 충돌
│  14:00  ⭐ 아티스트C - Sub Stage     │ ← 충돌
│                                     │
│  15:30  👋 A게이트에서 친구 만남     │
│         └ 내가 추가한 일정           │
│                                     │
│  16:00  ⭐ 아티스트D - Main Stage    │
└─────────────────────────────────────┘
```
...
**시간 충돌 표시**
- 동일 시간대 슬롯/이벤트 겹침 → 노란색 배경 + ⚠️ 아이콘
- 상단에 "충돌 N개" 배지
- 충돌 항목 탭 → 둘 중 하나 선택/해제 유도
...
### 5.9 크루 (Crew) — 커뮤니티/MyFes 확장 (P1)

> 번개 모집(일회성)과 구분되는 **지속적 모임** 기능

#### 5.9.1 크루 찾기 (커뮤니티 탭 내)
...
### 5.10 소셜 피드 — 홈 탭 확장 (P1)

> 친구의 활동을 보고 함께 공연 다닐 동기 부여
...
## 부록 B) 사용자 콘텐츠 등록 IA (P2)
> 상세 기획: `docs/proposals/user-event-registration.md`

### 행사 등록 플로우
```
탐색 페이지 → [+ 행사 등록] → 등록 폼 모달
                              ├─ 기본 정보 (제목, 일시, 장소, 유형)
                              ├─ 상세 정보 (포스터, 출연진, 가격, 예매링크)
                              └─ [등록하기] → 검토/게시
```

### 타임테이블 편집 플로우
```
행사 페이지 → 타임테이블 탭 → [수정 제안] 또는 [편집]
                              ├─ 슬롯 추가/수정/삭제
                              ├─ 스테이지 관리
                              ├─ 변경 사유 입력
                              └─ [제출] → 승인 대기 또는 즉시 반영
...
|------|----------|-----------|---------|
| 비로그인 | ❌ | ❌ (열람만) | ❌ (열람만) |
| 일반 사용자 | ✅ (검토 후 게시) | 수정 제안 | ✅ |
| 등록자/신뢰 사용자 | ✅ (즉시 게시) | 즉시 수정 | ✅ |
| 관리자 | ✅ | ✅ + 제안 승인 | ✅ |
