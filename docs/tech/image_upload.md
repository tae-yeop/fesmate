...
> 작성일: 2025-12-21
> 상태: ✅ 구현 완료 (Local Storage + Supabase Storage)
...
- [x] Supabase Storage 구현 (2025-12-31 완료)
...
| 버킷 | 용도 | 최대 크기 | 허용 타입 |
|------|------|----------|----------|
| `post-images` | 게시글 이미지 | 5MB | jpeg, png, gif, webp |
| `ticket-images` | 티켓북 이미지 | 5MB | jpeg, png, gif, webp |
| `avatars` | 프로필 이미지 | 2MB | jpeg, png, webp |

### RLS 정책
- **읽기**: 모든 버킷 public 접근 허용
- **쓰기**: 인증된 사용자만 업로드 가능
- **삭제**: 본인 폴더의 파일만 삭제 가능 (userId 기반 폴더 구조)
...
// 기본 (환경변수에 따라 local/supabase)
const storage = getImageStorage();

// Supabase 강제 지정
const supabaseStorage = getImageStorage("supabase", "post-images");

// 업로드
const result = await storage.upload(file, { maxWidth: 1200 });
if (result.success) {
